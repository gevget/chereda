...
export const profiles:Profile[]=heroRows.map(([slug,name,type,category,specialty,city,amount,unit],i)=>({id:slug,slug,name,type,category,subcategory:slug==='loft-bereg'?'Свадебные':slug==='sever-hall'?'Бизнес-площадки':slug==='elena-sokolova-hair'?'Стилисты по волосам':categories.find(c=>c.slug===category)!.subcategories[0],specialty,city,travel:type==='venue'?'local':slug==='elena-sokolova-hair'?'nearby':i%4===0?'countrywide':'local',price:p(amount,unit,city==='astana'?'KZT':city==='dubai'?'AED':'RUB'),bio:`${name} — ${specialty.toLowerCase()} для событий с продуманной эстетикой. В demo показаны услуги, условия работы и участие в общих проектах.`,styles:['editorial','botanical','современный'],verified:i%5!==2,rating:4.3+(i%7)*0.1,score:72000+i*1173,rank:i+1,capacity:slug==='loft-bereg'?150:slug==='sever-hall'?220:undefined,outsideCatering:type==='venue'?true:undefined,parkingSpaces:type==='venue'?(slug==='loft-bereg'?48:120):undefined,equipment:type==='venue'?['свет','звук','мебель','Wi‑Fi']:undefined,kitchen:type==='venue'?(slug==='loft-bereg'?'prep':'own'):undefined,accessibility:type==='venue'?['безбарьерный вход','лифт','санузел']:undefined,outdoor:type==='venue',accommodation:slug==='loft-bereg',venueType:type==='venue'?(slug==='loft-bereg'?'Лофт':'Бизнес-площадка'):undefined,catalogTags:type==='venue'?['Установка авто','Служебный вход','звуковое оборудование (да или нет)','Световое оборудование (да или нет)','Проекторы и экраны ( да или нет)']:['Есть фото','Есть Отзывы','Есть записи в портфолио'],projectIds:[],serviceIds:[],avatar:stockAvatarPool[i%stockAvatarPool.length],cover:stockCoverPool[i%stockCoverPool.length]}));
for(let i=18;i<72;i++){const cat=categories[(i-18)%categories.length];const stem=surnameStems[(i*7+Math.floor((i-18)/18))%18];const name=`${names[i%18]} ${stem}${i%2===0?'а':''}`;const subcategory=cat.subcategories.length?cat.subcategories[i%cat.subcategories.length]:cat.name;profiles.push({id:`profile-${i+1}`,slug:`profile-${i+1}`,name,type:'person',category:cat.slug,subcategory,specialty:subcategory,city:cities[i%cities.length].slug,travel:i%3===0?'countrywide':i%3===1?'nearby':'local',price:p(9000+(i%13)*14000,'проект'),bio:`${name} работает с событиями в категории «${cat.name}». В профиле собраны услуги, проекты и прозрачные условия выезда.`,styles:['современный',i%2?'editorial':'classic'],catalogTags:['Есть фото','Есть Отзывы','Есть записи в портфолио'],verified:i%3!==0,rating:4.3+(i%7)*0.1,score:65000+i*317,rank:i+1,projectIds:[],serviceIds:[],avatar:stockAvatarPool[i%stockAvatarPool.length],cover:stockCoverPool[i%stockCoverPool.length]});}
