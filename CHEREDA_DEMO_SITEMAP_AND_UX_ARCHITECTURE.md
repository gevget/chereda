...
- по одному представителю `/pro/[categorySlug]`, `/profiles/[profileSlug]`, `/projects/[projectSlug]` и `/collections/[collectionSlug]`; остальные записи остаются доступными из каталогов.

Карта сайта — каталог типов страниц для разработчика, а не полный перечень demo-записей. Для динамических сущностей в ней показывается по одному заполненному представителю: категория PRO, проект/кейс, профиль профессионала и коллекция. Все остальные записи открываются из соответствующих каталогов и списков. Smoke-тест проверяет доступность каждого зарегистрированного представительного href.