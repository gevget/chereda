...
export type Profile = {id:string;slug:string;name:string;type:'person'|'team'|'company'|'venue';category:string;subcategory:string;specialty:string;city:string;travel:'local'|'nearby'|'countrywide'|'international'|'remote';price:Price;bio:string;styles:string[];verified:boolean;rating:number;score:number;rank:number;capacity?:number;outsideCatering?:boolean;parkingSpaces?:number;equipment?:string[];kitchen?:'own'|'prep'|'none';accessibility?:string[];outdoor?:boolean;accommodation?:boolean;venueType?:string;catalogTags?:string[];projectIds:string[];serviceIds:string[];avatar:string;cover:string};
